...
** sly & slynk for debugging
   this config depends on [[https://github.com/joaotavora/sly][slynk]] to connect to a emacs with sly. unfortunately, slynk is
   not in quicklisp. the path of least resistance is to clone the sly repo into
   [[file:~/quicklisp/local-projects/][local-projects]]:
   #+begin_src sh
     git clone git@github.com:joaotavora/sly ~/quicklisp/local-projects/sly
   #+end_src
